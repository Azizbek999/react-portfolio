...
const Menu = () => {
    ...
    const [activeLink, setActiveLink] = useState(""); // Start with an empty initial value

    useEffect(() => {
        const currentPath = location.pathname.substring(1); // Remove leading '/'
        setActiveLink(currentPath); // Update activeLink based on current path
    }, [location]); // Re-run the effect whenever location changes


    return (
        <div className='navbar'>
            <Link className={activeLink === 'Home' ? 'navbar-link active' : 'navbar-link'} onClick={(e) => setActiveLink("Home")} to={"/Home"}>
            ...
            <Link className={activeLink === 'Resume' ? 'navbar-link active' : 'navbar-link'} onClick={(e) => setActiveLink("Resume")} to={"/Resume"}><span class="text-xl mb-1">
            ...
            <Link className={activeLink === 'Works' ? 'navbar-link active' : 'navbar-link'} onClick={(e) => setActiveLink("Works")} to={"/Works"}><span class="text-xl mb-1">
            ...
            <Link className={activeLink === 'Contact' ? 'navbar-link active' : 'navbar-link'} onClick={(e) => setActiveLink("Contact")} to={"/Contact"}>           <span class="text-xl mb-1">
